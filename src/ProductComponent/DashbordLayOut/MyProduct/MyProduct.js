...
const MyOrders = () => {
  ...
  useEffect(() => {
    fetch(
      `https://final-project-server-assignmen.vercel.app/deshbord/myorders/${user?.email}`
    )
      .then(res => res.json())
      .then(data => {
        setData(data);
  ...
  useEffect(() => {
    fetch(
      `https://final-project-server-assignmen.vercel.app/deshbord/myorhand/${user?.email}`
    )
      .then(res => res.json())
      .then(data => {
        console.log(data);
  ...
  const handleDeleteProduct = id => {
    fetch(
      `https://final-project-server-assignmen.vercel.app/SellarProduct/Delete/${id}`,
      {
        method: 'DELETE',
      }
    )
              ...
              <div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2">
                  ...
                  <p className="py-6 mr-2">
                    purchars Times : {addProduct?.Year}
                  </p>
                ...
                <div className="text-center my-6">
                  <button
                    onClick={() => handleDeleteProduct(addProduct?._id)}
