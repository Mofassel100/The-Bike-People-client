    ...
    <div className=" grid justify-center items-center bg-black text-white mx-2">
      <form onSubmit={handleSubmit(hadleLogingSubmit)}>
        ...
        <div className="form-control w-full max-w-xs">
          ...
          <input
            type="file"
            className="file-input file-input-bordered file-input-info text-black font-bold w-full max-w-xs"
            {...register('image')}
        ...
        <div className="form-control w-full max-w-xs">
          ...
          <input
            type="email"
            className="input text-black fond-bold text-2xl font-bold input-bordered w-full max-w-xs"
            {...register('email', { required: 'Email Address in required' })}
        ...
        <div className="form-control w-full max-w-xs">
          ...
          <input
            type="Password"
            className="input input-bordered w-full text-2xl font-bold max-w-xs text-black tont-bold"
            {...register('password', {
        ...
        <div className="text-center py-4">
          <select
            required
            className="px-5 font-bold w-full font-bold text-black py-3 tooltip tooltip-primary tooltip-top"
            data-tip="Choose Buyer OR Sellar"
            {...register('role')}
          >
            <option className="p-3 " value="buyer">
              Buyers
            </option>
            <option className="p-3" value="sellar">
              Sellars
            </option>
          </select>
        </div>
      ...
      <div className="flex text-center py-8 pl-10">
        {/* google login */}
        <div
          className="grid justify-center tooltip tooltip-primary tooltip-top items-center ml-11"
          data-tip="Google Login"
        >
          <button onClick={GooglesinInWithLogin}>
            {' '}
        ...
        <div
          className="grid justify-center tooltip tooltip-primary tooltip-top items-center ml-11"
          data-tip="FaceBook Login"
        >
          <button onClick={FaceboosinInWithLogin}>
            {' '}
            <BsFacebook className="text-5xl bg-blue-600 rounded " />{' '}
          </button>
          {/* <Link >  </Link> */}
        </div>
        {/* ----------------- */}
